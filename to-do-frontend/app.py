...
def submit():
    ...
    try:
        response = requests.post(BACKEND_URL + '/submittodoitem', json=dictionary_data, timeout=5)
#        response.raise_for_status()  # Raises HTTPError for 4xx/5xx status
    except requests.exceptions.RequestException as e:
        flash(f"Error submitting data: {str(e)}", "error")
        return render_template('to-do.html', form_data=dictionary_data)
...
def api():
    ...
    response_data['api_context']='Todo_Addition_List_API'
